/* gfxEs2FboDemo.inl - OpenGL ES2 Fbo Demo */
...
/*******************************************************************************
 *
 * es2FboCheckGL0 - check GL status
 *
...
#define es2FboCheckGL() es2FboCheckGL0(__FILE__, __LINE__)

/*******************************************************************************
 *
 * es2FboCheckFramebufferStatus0 - check framebuffer status
 *
 ...
 *
 */
static void es2FboCheckFramebufferStatus0
    (
    const char *file,
    int line
    )
...
#define es2FboCheckFramebufferStatus() \
        es2FboCheckFramebufferStatus0(__FILE__, __LINE__)

/*******************************************************************************
 *
 * es2FboPrint - print the configuration of the graphics library
 *
...
/*******************************************************************************
 *
 * es2FboCleanup - perform final cleanup
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: es2FboEglDeinit
 *
 */
static void es2FboCleanup
    (
    ES2FBO_SCENE* pDemo
    )
    {
...
/*******************************************************************************
 *
 * es2FboEglDeinit - perform necessary steps to deinitialize
 *
 * This routine takes care of all the steps involved in cleaning-up EGL
 * resources. The EGL resources must be provided by the ES2FBO_SCENE parameter
 * <pDemo>.
 *
 * RETURNS: N/A
 *
 * ERRNO: N/A
 *
 * SEE ALSO: es2FboEglInit
 *
 */
static void es2FboEglDeinit
    (
    ES2FBO_SCENE* pDemo
    )
    {
...
/*******************************************************************************
*
* es2FboFindDrmFb - find a DRM framebuffer
*
...
/*******************************************************************************
 *
 * es2FboEglInit - perform necessary steps to initialize
 *
 ...
 *
 * RETURNS: pointer to ES2FBO_SCENE, or NULL if the routine had to abort
 *
 * ERRNO: N/A
 *
 * SEE ALSO: es2FboEglDeinit
 *
 */
static ES2FBO_SCENE* es2FboEglInit
    (
    unsigned int device,
    ...
    pDemo = (ES2FBO_SCENE*)calloc (1, sizeof (ES2FBO_SCENE));
    if (pDemo == NULL)
        {
    ...
    if (printInfo)
        {
        es2FboPrint (pDemo, eglMajor, eglMinor);
        }
...
/*******************************************************************************
 *
 * es2FboDeinit - deinitialize the application
 *
...
/*******************************************************************************
 *
 * es2FboInit - initialize the application
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: es2FboDeinit
 *
 */
static int es2FboInit
    (
    ES2FBO_SCENE* pDemo
    )
    {
...
/*******************************************************************************
 *
 * es2FboRender - render the application
 *
...
/*******************************************************************************
 *
 * es2FboPerfCount - calculate and show performance
 *
...
/*******************************************************************************
 *
 * es2FboGetChar - gets an ASCII character from keyboard
 *
 ...
 *
 */
static unsigned short es2FboGetChar
    (
    int evDevFd
    )
    {
...
/*******************************************************************************
 *
 * gfxEs2FboDemo - core functionality of the program
 *
...
static int gfxEs2FboDemo
...
#endif
    /* Initialize demo */
    pDemo = es2FboEglInit (device, eglDriver);
    if (pDemo == NULL)
        {
        (void)fprintf (stderr, "es2FboEglInit failed\n");
        return (EXIT_FAILURE);
        }
    ...
    if (es2FboInit (pDemo))
        {
        (void)fprintf (stderr, "es2FboInit failed\n");
        es2FboDeinit (pDemo);
        es2FboEglDeinit (pDemo);
        return (EXIT_FAILURE);
        }
    ...
    while (((diffTime < runTime)||(runTime == 0))
        ...
        if (pDemo->evDevFd > 0)
            {
            unsigned short character = es2FboGetChar (pDemo->evDevFd);
            if (character == (unsigned short)'q') kbdQuit = 1;
            }
#endif
        es2FboPerfCount (pDemo);

        if (es2FboRender (pDemo))
            {
            (void)fprintf (stderr, "es2FboRender failed\n");
            break;
            }
