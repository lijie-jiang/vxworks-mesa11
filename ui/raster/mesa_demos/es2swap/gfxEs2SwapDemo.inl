/* gfxEs2SwapDemo.inl - OpenGL ES2 Swap Demo */
...
13nov14,yat  Created OpenGL ES2 swap demo (US46449)
*/
...
/*******************************************************************************
 *
 * es2SwapPrint - print the configuration of the graphics library
 *
...
/*******************************************************************************
 *
 * es2SwapCleanup - perform final cleanup
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: es2SwapEglDeinit
 *
 */
static void es2SwapCleanup
    (
    ES2SWAP_SCENE* pDemo
    )
    {
...
/*******************************************************************************
 *
 * es2SwapEglDeinit - perform necessary steps to deinitialize
 *
 * This routine takes care of all the steps involved in cleaning-up EGL
 * resources. The EGL resources must be provided by the ES2SWAP_SCENE parameter
 * <pDemo>.
 *
 * RETURNS: N/A
 *
 * ERRNO: N/A
 *
 * SEE ALSO: es2SwapEglInit
 *
 */
static void es2SwapEglDeinit
    (
    ES2SWAP_SCENE* pDemo
    )
    {
    ...
    es2SwapCleanup (pDemo);
    }

#if defined(GFX_USE_GBM)
/*******************************************************************************
*
* es2SwapFindDrmFb - find a DRM framebuffer
*
...
/*******************************************************************************
 *
 * es2SwapEglInit - perform necessary steps to initialize
 *
 ...
 *
 * RETURNS: pointer to ES2SWAP_SCENE, or NULL if the routine had to abort
 *
 * ERRNO: N/A
 *
 * SEE ALSO: es2SwapEglDeinit
 *
 */
static ES2SWAP_SCENE* es2SwapEglInit
    (
    unsigned int device,
    ...
    pDemo = (ES2SWAP_SCENE*)calloc (1, sizeof (ES2SWAP_SCENE));
    if (pDemo == NULL)
        {
    ...
    if (printInfo)
        {
        es2SwapPrint (pDemo, eglMajor, eglMinor);
        }
...
/*******************************************************************************
 *
 * es2SwapDeinit - deinitialize the application
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: es2SwapInit
 *
 */
static void es2SwapDeinit
    (
    ES2SWAP_SCENE* pDemo
    )
    {
...
/*******************************************************************************
 *
 * es2SwapInit - initialize the application
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: es2SwapDeinit
 *
 */
static int es2SwapInit
    (
    ES2SWAP_SCENE* pDemo
    )
    {
...
/*******************************************************************************
 *
 * es2SwapRender - render the application
 *
...
/*******************************************************************************
 *
 * es2SwapPerfCount - calculate and show performance
 *
...
/*******************************************************************************
 *
 * es2SwapGetChar - gets an ASCII character from keyboard
 *
 ...
 *
 */
static unsigned short es2SwapGetChar
    (
    int evDevFd
    )
    {
...
/*******************************************************************************
 *
 * gfxEs2SwapDemo - core functionality of the program
 *
...
static int gfxEs2SwapDemo
...
#endif
    /* Initialize demo */
    pDemo = es2SwapEglInit (device, eglDriver);
    if (pDemo == NULL)
        {
        (void)fprintf (stderr, "es2SwapEglInit failed\n");
        return (EXIT_FAILURE);
        }
    ...
    /* Initialize demo */
    if (es2SwapInit (pDemo))
        {
        (void)fprintf (stderr, "es2SwapInit failed\n");
        es2SwapDeinit (pDemo);
        es2SwapEglDeinit (pDemo);
        return (EXIT_FAILURE);
        }
    ...
    while (((diffTime < runTime)||(runTime == 0))
        ...
        if (pDemo->evDevFd > 0)
            {
            unsigned short character = es2SwapGetChar (pDemo->evDevFd);
            if (character == (unsigned short)'q') kbdQuit = 1;
            }
#endif
        es2SwapPerfCount (pDemo);

        if (es2SwapRender (pDemo))
            {
            (void)fprintf (stderr, "es2SwapRender failed\n");
            break;
            }
