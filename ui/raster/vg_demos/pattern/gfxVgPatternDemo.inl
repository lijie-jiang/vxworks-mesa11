/* gfxVgPatternDemo.inl - Vector Graphics Tiling Demo */
...
This example program demonstrates the concepts in loading and tiling an image.
...
#define VG_PATTERN_ERROR_EGL           0
#define VG_PATTERN_ERROR_OPENVG        1
...
typedef struct
    ...
    long                 tRate0;
    long                 frames;
    VGImage              image;      /* VG tiling image */
    VGPaint              paint;      /* VG paint color */
    VGPath               path;       /* VG tiling path */
    } VG_PATTERN_SCENE;

/*******************************************************************************
 *
 * vgPatternReportError - reports on an EGL or Vector Graphics error
 *
 ...
 *
 */
static void vgPatternReportError
    (
    ...
    else if (type == VG_PATTERN_ERROR_OPENVG)
        {
        errNum = vgGetError ();
...
/*******************************************************************************
 *
 * vgPatternCleanup - perform final cleanup
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: vgPatternEglDeinit
 *
 */
static void vgPatternCleanup
    (
    VG_PATTERN_SCENE* pDemo
    )
    {
#if defined(GFX_USE_EVDEV)
    if (pDemo->evDevFd > 0)
...
/*******************************************************************************
 *
 * vgPatternEglDeinit - perform necessary steps to deinitialize
 *
 * This routine takes care of all the steps involved in cleaning-up EGL
 * resources. The EGL resources must be provided by the VG_PATTERN_SCENE parameter
 * <pDemo>.
 *
 * RETURNS: N/A
 *
 * ERRNO: N/A
 *
 * SEE ALSO: vgPatternEglInit
 *
 */
static void vgPatternEglDeinit
    (
    VG_PATTERN_SCENE* pDemo
    )
    {
    ...
    vgPatternCleanup (pDemo);
    }

/*******************************************************************************
 *
 * vgPatternEglInit - perform necessary steps to initialize
 *
 ...
 *
 * RETURNS: pointer to VG_PATTERN_SCENE, or NULL if the routine had to abort
 *
 * ERRNO: N/A
 *
 * SEE ALSO: vgPatternEglDeinit
 *
 */
static VG_PATTERN_SCENE* vgPatternEglInit
    (
    unsigned int device
    )
    {
    ...
    pDemo = (VG_PATTERN_SCENE*)calloc (1, sizeof(VG_PATTERN_SCENE));
    if (pDemo == NULL)
        {
    ...
    if (pDemo->display == EGL_NO_DISPLAY)
        ...
        vgPatternReportError ("eglGetDisplay", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }

    if (!eglInitialize (pDemo->display, &eglMajor, &eglMinor))
        {
        vgPatternReportError ("eglInitialize", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }
    ...
    if (!eglBindAPI (EGL_OPENVG_API))
        {
        vgPatternReportError ("eglBindAPI", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }
    /*
    ...
    if (!eglChooseConfig (pDemo->display, configAttribs, &eglConfig, 1, &numConfigs) || (numConfigs <= 0))
        ...
        vgPatternReportError ("eglChooseConfig", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }
    ...
    if (pDemo->surface == EGL_NO_SURFACE)
        {
        vgPatternReportError ("eglCreateWindowSurface", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }
    ...
    if (pDemo->context == EGL_NO_CONTEXT)
        {
        vgPatternReportError ("eglCreateContext", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }
    ...
    if (!eglMakeCurrent (pDemo->display, pDemo->surface, pDemo->surface, pDemo->context))
        {
        vgPatternReportError ("eglMakeCurrent", VG_PATTERN_ERROR_EGL);
        vgPatternEglDeinit (pDemo);
        return (NULL);
        }
...
/*******************************************************************************
 *
 * vgPatternDeinit - deinitialize the application
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: vgPatternInit
 *
 */
static void vgPatternDeinit
    (
    VG_PATTERN_SCENE* pDemo
    )
    {
    vgSetPaint (VG_INVALID_HANDLE, VG_FILL_PATH | VG_STROKE_PATH);
    if (pDemo->path != VG_INVALID_HANDLE)
        {
        vgDestroyPath (pDemo->path);
        }
    if (pDemo->paint != VG_INVALID_HANDLE)
        {
        vgDestroyPaint (pDemo->paint);
        }
    if (pDemo->image != VG_INVALID_HANDLE)
        {
        vgDestroyImage (pDemo->image);
        }
    }

/*******************************************************************************
 *
 * vgPatternInit - initialize the application
 *
 ...
 * ERRNO: N/A
 *
 * SEE ALSO: vgPatternDeinit
 *
 */
static int vgPatternInit
    (
    VG_PATTERN_SCENE* pDemo
    )
    {
    VGubyte segs[11] = {0};                 /* VG segments data */
    VGfloat data[20] = {0.0};               /* VG path coordinates */
    const float blue[4] = {0.0, 0.0, 1.0, 1.0};

    /* Create the image that'll be used as a paint */
    pDemo->image = vgCreateImage (VG_sARGB_8888_PRE, starWidth, starHeight, VG_IMAGE_QUALITY_BETTER);
    if (pDemo->image == VG_INVALID_HANDLE)
        {
        vgPatternReportError ("vgCreateImage", VG_PATTERN_ERROR_OPENVG);
        return FALSE;
        }
    vgImageSubData(pDemo->image, starData, starStride, starFormat, 0, 0,
    ...
    if (pDemo->paint == VG_INVALID_HANDLE)
        {
        vgPatternReportError ("vgCreatePaint", VG_PATTERN_ERROR_OPENVG);
        return FALSE;
        }
    vgSetfv (VG_TILE_FILL_COLOR, 4, blue);

    /* Manipulate the paint to associate with the image */
    vgSeti (VG_MATRIX_MODE, VG_MATRIX_FILL_PAINT_TO_USER);
    vgLoadIdentity ();
    vgSetParameteri (pDemo->paint, VG_PAINT_TYPE, VG_PAINT_TYPE_PATTERN);
    vgSetParameteri (pDemo->paint, VG_PAINT_PATTERN_TILING_MODE, VG_TILE_REPEAT);
    vgPaintPattern (pDemo->paint, pDemo->image);

    /* Create star shape */
    pDemo->path = vgCreatePath (VG_PATH_FORMAT_STANDARD, VG_PATH_DATATYPE_F,
                         1.0, 0.0, 0, 0, VG_PATH_CAPABILITY_ALL);
    if (pDemo->path == VG_INVALID_HANDLE)
        {
        vgPatternReportError ("vgCreatePath", VG_PATTERN_ERROR_OPENVG);
        return FALSE;
        }

    segs[0] = VG_MOVE_TO_ABS;
    segs[1] = VG_LINE_TO_ABS;
    segs[2] = VG_LINE_TO_ABS;
    segs[3] = VG_LINE_TO_ABS;
    segs[4] = VG_LINE_TO_ABS;
    segs[5] = VG_LINE_TO_ABS;
    segs[6] = VG_LINE_TO_ABS;
    segs[7] = VG_LINE_TO_ABS;
    segs[8] = VG_LINE_TO_ABS;
    segs[9] = VG_LINE_TO_ABS;
    segs[10] = VG_CLOSE_PATH;

    data[0] = 0.0f;
    data[1] = 0.6125f*(VGfloat)pDemo->surfHeight;
    data[2] = 0.3875f*(VGfloat)pDemo->surfWidth;
    data[3] = 0.6125f*(VGfloat)pDemo->surfHeight;
    data[4] = 0.5f   *(VGfloat)pDemo->surfWidth;
    data[5] = (VGfloat)pDemo->surfHeight;
    data[6] = 0.6125f*(VGfloat)pDemo->surfWidth;
    data[7] = 0.6125f*(VGfloat)pDemo->surfHeight;
    data[8] = (VGfloat)pDemo->surfWidth;
    data[9] = 0.6125f*(VGfloat)pDemo->surfHeight;
    data[10] = 0.6825f*(VGfloat)pDemo->surfWidth;
    data[11] = 0.3875f*(VGfloat)pDemo->surfHeight;
    data[12] = 0.8125f*(VGfloat)pDemo->surfWidth;
    data[13] = 0.0f;
    data[14] = 0.5f   *(VGfloat)pDemo->surfWidth;
    data[15] = 0.25f  *(VGfloat)pDemo->surfHeight;
    data[16] = 0.1875f*(VGfloat)pDemo->surfWidth;
    data[17] = 0.0f;
    data[18] = 0.3175f*(VGfloat)pDemo->surfWidth;
    data[19] = 0.3875f*(VGfloat)pDemo->surfHeight;

    vgAppendPathData (pDemo->path, sizeof(segs), segs, data);
    if (vgGetError () != VG_NO_ERROR)
        {
        vgPatternReportError ("vgAppendPathData", VG_PATTERN_ERROR_OPENVG);
        return FALSE;
        }
...
/*******************************************************************************
 *
 * vgPatternRender - render the application
 *
...
static int vgPatternRender
    ...
    vgLoadIdentity ();
    vgDrawPath (pDemo->path, VG_FILL_PATH);
...
/*******************************************************************************
 *
 * vgPatternPerfCount - calculate and show performance
 *
 ...
 *
 */
static void vgPatternPerfCount
    (
    VG_PATTERN_SCENE* pDemo
    )
    {
...
/*******************************************************************************
 *
 * vgPatternGetChar - gets an ASCII character from keyboard
 *
 ...
 *
 */
static unsigned short vgPatternGetChar
    (
    int evDevFd
    )
    {
...
/*******************************************************************************
 *
 * gfxVgPatternDemo - core functionality of the program
 *
...
static int gfxVgPatternDemo
...
#endif
    /* Initialize demo */
    pDemo = vgPatternEglInit (device);
    if (pDemo == NULL)
        {
        (void)fprintf (stderr, "vgPatternEglInit failed\n");
        return (EXIT_FAILURE);
        }

    /* Initialize demo */
    if (vgPatternInit (pDemo) == FALSE)
        {
        (void)fprintf (stderr, "vgPatternInit failed\n");
        vgPatternDeinit (pDemo);
        vgPatternEglDeinit (pDemo);
        return (EXIT_FAILURE);
        }
    ...
    while (((diffTime < runTime)||(runTime == 0))
        ...
        if (pDemo->evDevFd > 0)
            {
            unsigned short character = vgPatternGetChar (pDemo->evDevFd);
            if (character == (unsigned short)'q') kbdQuit = 1;
            }
#endif
        vgPatternPerfCount (pDemo);

        if (vgPatternRender (pDemo) != TRUE)
            {
            (void)fprintf (stderr, "vgPatternRender failed\n");
            vgPatternCleanup (pDemo);
            return (EXIT_FAILURE);
            }
